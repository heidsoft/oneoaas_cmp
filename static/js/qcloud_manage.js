...
//vcenter 管理插件
var QcloudManage = (function ($,toastr) {
    return {
        //初始化虚拟机表格对象
        vmTable:null,
        //被选择记录ID
        ...
        init:function (tableId) {
            ...
            var QcloudManageRecord = $(tableId).DataTable({
                paging: true, //隐藏分页
                ordering: false, //关闭排序
            ...
            this.vmTable = QcloudManageRecord;

            //设置button
            new $.fn.dataTable.Buttons( QcloudManageRecord, {
                buttons: [
                    {
                        extend: 'copyHtml5',
                        text: '拷贝表格'
            ...
            //将button放置到底部
            var tableContainer = QcloudManageRecord.buttons().container();
            tableContainer.appendTo(
                QcloudManageRecord.table().container()
            );

            return QcloudManageRecord;
        }
    }
})($,window.toastr);
...
$(document).ready(function(){
    ...
    QcloudManage.init('#qcloud_manage_record');

    $('#host_manage_record').on( 'click', 'tr:gt(0)', function () {
        var currentHost = QcloudManage.vmTable.row( this ).data();
