...
//vcenter 管理插件
var UcloudManage = (function ($,toastr) {
    return {
        //初始化虚拟机表格对象
        vmTable:null,
        //被选择记录ID
        ...
        init:function (tableId) {
            ...
            var UcloudManageRecord = $(tableId).DataTable({
                paging: true, //隐藏分页
                ordering: false, //关闭排序
            ...
            this.vmTable = UcloudManageRecord;

            //设置button
            new $.fn.dataTable.Buttons( UcloudManageRecord, {
                buttons: [
                    {
                        extend: 'copyHtml5',
                        text: '拷贝表格'
            ...
            //将button放置到底部
            var tableContainer = UcloudManageRecord.buttons().container();
            tableContainer.appendTo(
                UcloudManageRecord.table().container()
            );

            return UcloudManageRecord;
        }
    }
})($,window.toastr);

//扩展到jquery
//$.fn.extend(UcloudManage);
//扩展函数
...
$(document).ready(function(){
    ...
    UcloudManage.init('#ucloud_manage_record');

    $('#host_manage_record').on( 'click', 'tr:gt(0)', function () {
        var currentHost = UcloudManage.vmTable.row( this ).data();
